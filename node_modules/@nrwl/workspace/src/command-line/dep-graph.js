...
function generateGraph(args, affectedProjects) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        let graph = project_graph_1.pruneExternalNodes(yield project_graph_1.createProjectGraphAsync());
        ...
        let html;
        if (!args.file || path_2.extname(args.file) === '.html') {
        ...
        if (args.file) {
            ...
            const ext = path_2.extname(args.file);
            const fullFilePath = path_2.isAbsolute(args.file)
                ? args.file
                : path_2.join(workspaceFolder, args.file);
            const fileFolderPath = path_2.dirname(fullFilePath);
            if (ext === '.html') {
                const assetsFolder = path_2.join(fileFolderPath, 'static');
                const assets = [];
                ...
                fs_1.writeFileSync(fullFilePath, html);
                ...
                fs_extra_1.ensureDirSync(path_2.dirname(fullFilePath));
                fileutils_1.writeJsonFile(fullFilePath, {
                    graph,
                    affectedProjects,
            ...
            yield startServer(html, args.host || '127.0.0.1', args.port || 4211, args.watch, affectedProjects, args.focus, args.groupByFolder, args.exclude, args.open);
        }
    });
}
exports.generateGraph = generateGraph;
function startServer(html, host, port = 4211, watchForchanges = false, affected = [], focus = null, groupByFolder = false, exclude = [], openBrowser = true) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        if (watchForchanges) {
            startWatcher();
        }
