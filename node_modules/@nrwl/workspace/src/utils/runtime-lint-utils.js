...
function isRelativeImportIntoAnotherProject(imp, projectPath, projectGraph, sourceFilePath, sourceProject) {
    if (!isRelative(imp))
        return false;
    const targetFile = devkit_1.normalizePath(path.resolve(path.join(projectPath, path.dirname(sourceFilePath)), imp)).substring(projectPath.length + 1);
...
function findTargetProject(projectGraph, targetFile) {
    ...
    if (!targetProject) {
        targetProject = findProjectUsingFile(projectGraph, devkit_1.normalizePath(path.join(targetFile, 'index')));
    }
    if (!targetProject) {
        targetProject = findProjectUsingFile(projectGraph, devkit_1.normalizePath(path.join(targetFile, 'src', 'index')));
    }
...
function getSourceFilePath(sourceFileName, projectPath) {
    return devkit_1.normalizePath(sourceFileName).substring(projectPath.length + 1);
}
...
function hasBannedImport(source, target, depConstraints) {
    ...
    return depConstraints.find((constraint) => constraint.bannedExternalImports.includes(target.data.packageName));
}
